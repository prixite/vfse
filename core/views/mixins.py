...
class UserMixin:
    ...
    def add_sites(self, data, user_id):
        sites = [models.UserSite(user_id=user_id, site=site,organization=data['organization']) for site in data["sites"]]
        models.UserSite.objects.bulk_create(sites)

    def add_modalities(self, data, user_id):
        modalities = [
            models.UserModality(user_id=user_id, modality=modality,organization=data['organization'])
