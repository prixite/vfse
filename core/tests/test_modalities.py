...
class ModalityTestCase(BaseTestCase):
    ...
    def test_modalitiy_manufacturers_list(self):
        ...
        self.assertEqual(response.status_code,200)
        self.assertEqual(len(response.json()),models.Manufacturer.objects.filter(id__in=models.ProductModel.objects.filter(modality=self.modality).values_list('product__manufacturer')).count())