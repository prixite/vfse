...
class PermissionTestCase(BaseTestCase):
    ...
    def test_super_user_permissions(self):
        ...
        paths = [
            str(path.pattern) for path in urlpaths.url_patterns if str(path.pattern).startswith('api/')
        ]
        ...
        for url in paths:
            url_path = re.sub("<([\w:]+)>","1",str(url))
            response = self.client.patch(f"/{url_path}",data={'users':[],'health_networks':[],'email':''})
            self.assertNotEqual(response.status_code,403)
        print('completed successfully.')
        ...
        for url in paths:
            url_path = re.sub("<([\w:]+)>","1",str(url))
            response = self.client.put(f"/{url_path}",data={'users':[],'health_networks':[]})
            self.assertNotEqual(response.status_code,403)
        print('completed successfully.')
