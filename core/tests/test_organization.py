...
class OrganizationTestCase(BaseTestCase):
    ...
    def test_user_upsert(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(models.UserModality.objects.filter(user__username=user_data['email']).exists(),True)
        self.assertEqual(models.UserSite.objects.filter(user__username=user_data['email']).exists(),True)
        self.assertEqual(models.Membership.objects.filter(user__username=user_data['email'],role=user_data['role']).exists(),True)
