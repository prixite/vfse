...
class ManufacturerTestCase(BaseTestCase):
    ...
    def test_list_manufacturer_images(self):
        ...
        self.assertEqual(response.status_code,200)
        self.assertEqual(len(response.json()),models.ManufacturerImage.objects.all().count())
    ...
    def test_create_manufacturer_images(self):
        ...
        self.assertEqual(response.status_code,201)
        self.assertTrue(models.ManufacturerImage.objects.filter(image='http://example.com/newimage.jpeg').exists())
