...
const  SystemImageGallery= () => {
    const [selectedImage, setSelectedImage] = useState(''); // eslint-disable-next-line
          ...
          >
            {
              data.map((image, imgIndex)=>(
                  <div className="image" onClick={()=> handleSelectedImage(imgIndex, image.image)}>
                    <img src={image.image} style={{border: `${index === imgIndex ? '5px solid rgb(119, 60, 189)' : ''}` }}/> 
                 </div>
              ))
            }
         </Flicking>
