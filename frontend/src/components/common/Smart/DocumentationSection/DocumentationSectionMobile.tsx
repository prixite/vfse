...
const DocumentationSectionMobile = ({docList ,documentationLink,openDropDown} : DocumentationSectionMobileProps) => {
  const [slicePointer,setSlicePointer] = useState(0);
    ...
    const fetchMoreSection = () => {
      setSlicePointer((prevState)=>prevState + 2);
       ...
       setTimeout(() => {
          setPaginatedDocs((prevState)=>[...prevState.concat([...docList.slice(slicePointer,slicePointer+2)])]);
        }, 500);
  }
  useEffect(()=>{
    ...
    <Box className="DocumentationSectionMobile">
           ...
           >
      {
        paginatedDocs?.map((doc,i)=>
         <DocumentationCard key={i} doc={doc} documentationLink={documentationLink} openDropDown={openDropDown}/>
        )
      }
         </InfiniteScroll>
    </Box>
