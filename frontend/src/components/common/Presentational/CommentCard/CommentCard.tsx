...
const CommentCard = ({ comment ,userId,refetchNotes }: CommentProps) => {
  ...
  const handleNoteDelete = async () => {
    await deleteSystemNoteService (comment?.id,deleteNote,refetchNotes )
    .catch(()=>{
  ...
  return (
    ...
    <div className="CommentCard">
      ...
      <div className = "userActions">
        { !editMode ?
        <>
        <img src={EditLogo} className="logo deleteLogo" onClick = {()=>setEditMode(true)}/>
        <img src={DeleteLogo} className="logo deleteLogo" onClick={()=>setOpenModal(true)} />
        </>
        :     
         <CloseIcon
      ...
      :
      <EditComment note ={comment?.note} noteId = {comment?.id} refetchNotes = {refetchNotes} setEditMode = {setEditMode}/>
    }
    </div>
    <DeleteNoteModal handleNoteDelete={handleNoteDelete} open = {openModal} handleClose={handleClose}/>
    </>
  );
};
