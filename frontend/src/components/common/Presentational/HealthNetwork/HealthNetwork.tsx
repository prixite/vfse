...
const HealthNetwork = ({index,setNetworks , network , allNetworks }: HealthNetworkProps) => {
  const constantData: object = localizedData()?.healthNetwork;
  const { name, logo } = constantData;
  const [selectedImage , setSelectedImage] = useState([]);

  const handleNameChange = (event) => {
    let TempNetworks = [...allNetworks];
    ...
    <div className="health-section">
      ...
      <div className="health-info">
        <div style={{ width: "100%", marginTop: "25px" }}>
          ...
          <DropzoneBox imgSrc={network?.appearance?.logo} setSelectedImage={setSelectedImage} />
        </div>
      </div>
    </div>
  );
