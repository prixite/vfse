...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def get_organization(self):
        org_id = self.get_value("organization_id")
        if not org_id or not org_id.isdigit():
            raise InvalidValueError("Invalid Organization id, must be an integer")
        return org_id

    def get_system(self):
        system_id = self.get_value("system_id")
        if not system_id or not system_id.isdigit():
            raise InvalidValueError("Invalid System id, must be an integer")
        return system_id
    ...
    def get_default_encoding(self, ssh):
        ...
        for command in commands:
            try:
                _, stdout, _ = ssh.exec_command(  # nosec
